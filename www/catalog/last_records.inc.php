...
if (mysql_num_rows($result)) {
	while(($notice = mysql_fetch_object($result))) {
		if (($notice->niveau_biblio =='s' || $notice->niveau_biblio =='a') && ($notice->niveau_hierar== 1 || $notice->niveau_hierar== 2)) {
			...
			// SENIA
			$numbanc = "SELECT COUNT(expl_notice) FROM exemplaires";
			$numbanc.= " WHERE expl_notice = '".$notice->notice_id."'";
			$result_num = mysql_query($numbanc, $dbh);
	
			$link = './catalog.php?categ=isbd&id=!!id!!';
			//$link = './catalog.php?categ=isbd&id=!!id!!'.$result_num;
