...
if ($nex->explr_acces_autorise!="INVIS") {
	...
	if ($nex->explr_acces_autorise=="MODIF") {
		...
		$dupliquer = "&nbsp;<input type='button' class='bouton' value=\"".$msg['dupl_expl_bt']."\" name='dupl_ex' id='dupl_ex' onClick=\"unload_off();document.location='./catalog.php?categ=dupl_expl&id=$id&cb=".urlencode($cb)."&expl_id=".$expl_id."' ; \" />
			";
		$dupliquer_bllibres = "&nbsp;<input type='button' class='bouton' value='Duplicar Banc Llibres' name='dupl_ex_bllibres' id='dupl_ex_bllibres' onClick=\"unload_off();document.location='./catalog.php?categ=dupl_expl_bllibres&id=$id&cb=".urlencode($cb)."&expl_id=".$expl_id."' ; \" />
			";
		// lien pour la modification
		$modifier = "<input type='submit' class='bouton' value=' $msg[77] ' onClick=\"unload_off();return test_form(this.form);\" />".$dupliquer.$dupliquer_bllibres;
	} else {
		$modifier="";
		$supprimer="";
	}
