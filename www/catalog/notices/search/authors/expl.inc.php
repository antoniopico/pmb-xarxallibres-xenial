...
//rellena ceros
		...
		if(pmb_mysql_num_rows($myQuery)==0) {
			...
			$requete.= $acces_j;
			$requete.= "left join bulletins on bulletin_notice=notice_id left join exemplaires on (bulletin_id=expl_bulletin and expl_notice=0) ";
			$requete.= "WHERE niveau_biblio='s' AND (exemplaires.expl_cb like '$code' OR bulletin_numero like '$code' OR bulletin_cb like '$code' OR notices.code like '".f_rellena_ceros($code)."')  $where_typedoc ";
// ----------------------------- FIN LLIUREX 09/03/2016 ------------------------------------			
			$requete.= "GROUP BY bulletin_id ".$limit_page;
			$rqt_bulletin=1;
		}
