...
function cambiarEstado($expl_cb,$expl_statut){

	$link2 = @mysql_connect(SQL_SERVER, USER_NAME, USER_PASS) OR die("Error MySQL");

	$q = "UPDATE exemplaires SET expl_statut= \"$expl_statut\" where expl_cb= \"$expl_cb\";";
	
	$resultData = @mysql_query($q, $link2);
	
	echo ("{"); 
	echo ("expl_cb : ".$expl_cb.",");
	echo ("expl_statut : ".$expl_statuti.",");
	echo ("sql: ".$q.",");
	echo ("sql_result: ".$resultData);
	echo ("}");


}

// Obtenemos expl_id from GET
$expl_cb = $_GET['expl_cb'];
$expl_statut = $_GET['expl_statut'];
