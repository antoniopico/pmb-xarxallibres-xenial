...
// -------------- LLIUREX 09/03/2016------------------------------
// ya no és necesario comprobar ceros
...
if (!$back_to_visu){
	...
	if($form_cb_expl){
// ------------------- LLIUREX 09/03/2016 -------------------
//cambiamos el comodin por defecto
		if($form_cb_expl=='*') $form_cb_expl='%';
//sustituimos por like para obtener valores aproximados
...
// ya no és necesario comprobar ceros
		if(!pmb_mysql_num_rows($result)) {
			//$form_cb_expl=f_rellena_ceros($form_cb_expl);
			$query = "select expl_id, expl_notice, pret_flag, pret_idempr from docs_statut, exemplaires left join pret on pret_idexpl=expl_id where expl_cb like '$form_cb_expl' and expl_statut=idstatut ";
			$result = pmb_mysql_query($query, $dbh);
		}
// -------------- FIN LLIUREX 09/03/2016--------------
		if(!pmb_mysql_num_rows($result)) {
